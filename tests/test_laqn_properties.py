...
Tests that the LAQN metadata normalisation pipeline produces conformant
output regardless of the shape or content of upstream API responses.

The data path is shared with ``regulatory.py`` (RData via the openair feed)
and is covered by ``test_regulatory.py`` and ``test_laqn.py``.
"""
...
from aeolus.sources.laqn import fetch_laqn_metadata
...
def laqn_site_dicts(draw, min_size=1, max_size=20):
    """Generate a list of LAQN ERG API site metadata dicts."""
