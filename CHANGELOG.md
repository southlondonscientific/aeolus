...
- **AirQo and PurpleAir `created_at` was frozen at module-import time** — both adapters passed `datetime.now(timezone.utc)` as a static value into `add_column`, and because their normalisers are constructed once at `register_source()` (i.e. at module import), every fetched row carried the import timestamp instead of the actual fetch time. The audit column "when did I download this row" was wrong for these two sources. Now passed as a lambda so each fetch evaluates fresh. **Migration**: if you rely on `created_at` for cache-staleness checks or download-provenance audit on AirQo / PurpleAir data, re-fetch.
