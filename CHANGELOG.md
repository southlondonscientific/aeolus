...
- **EEA `ratification` labels were inverted** — the EIONET vocabulary is 1 = Verified, 2 = Preliminary verified, 3 = Not verified; Aeolus mapped 1 → `Provisional` and 2/3 → `Verified`. Because only the up-to-date feed is queried (all code 2), every EEA row was labelled `Verified` and is now `Provisional`. Anything filtering EEA data on `ratification == "Verified"` was selecting exactly the unverified rows.
