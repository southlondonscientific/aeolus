...
- **SOS rows carried the unit `ug/m-3`** — the near-real-time UK sources (`AURN-SOS` etc., and therefore `get_current()`) built their unit from the upstream `ug.m-3` with a half-right string replace, and the shipped station mapping holds 1,345 such entries. Nothing downstream recognises `ug/m-3`: metrics warned "unknown unit" and treated CO `mg/m-3` as µg/m³. All unit strings now go through one canonicaliser (`aeolus.units.canonical_unit`), used by every adapter and by the metrics.
...
- **`aq_stats()` ignored the `units` column** — exceedance thresholds are defined in µg/m³ but were applied to the raw value, so NO2 in ppb (AirNow, some OpenAQ) never exceeded 200 and `annual_mean` was ppb labelled as µg/m³. Values in mg/m³, ppb and ppm are now converted first. ppb and ppm are converted; CO in mg/m³ stays in mg/m³, as LAQM reports it. `aq_stats()` output gains a **`units` column** saying which unit each row's statistics are in.
...
- **EEA timestamps were an hour late, and not timezone-aware.** The EEA download service publishes hourly `Start`/`End` in fixed UTC+1 (its own page says so; confirmed against the German UBA API and Luchtmeetnet). Aeolus passed them through as if UTC. They are now converted, and `date_time` is tz-aware like every other source. Italy's up-to-date feed is on local clock time instead and is treated as Europe/Rome.
- **OpenAQ values were labelled with the END of their hour**, so the same hour from OpenAQ and from AURN sat one hour apart. OpenAQ now uses the start of the period, like every other source.
- **Sonitus values were labelled with the end of each 15-minute bin** (now the start), and **Sonitus downloads lost their first hour in summer** — the server filters by Dublin local time. Aeolus now requests an hour either side and trims to the requested window. After these fixes Sonitus and its EEA Irish twins agree at zero lag, r = 1.0000.
- **`get_current("AIRNOW")` was off by the site's UTC offset.** AirNow's current-observation endpoint gives a local date and hour plus a zone abbreviation, which was read as UTC.
- **Request windows depended on the machine's time zone, or mislabelled aware datetimes.** PurpleAir and Sonitus converted naive dates with `.timestamp()` (a New York machine asked for a window five hours off); Breathe London, EEA, SOS and AirQo formatted timezone-aware non-UTC dates with a literal `Z`. All request windows now go through `aeolus._dates.to_utc()`: naive means UTC, aware is converted.
- `LAQN-ERG` returned whole days regardless of the requested hours; it now trims to the window, as `LAQN` does.
- **LAQN gases were ppb labelled `ug/m3` — values roughly half what they should be.** The londonair openair files (`aeolus.download("LAQN", ...)`) store gases in volume units (ppb; CO in ppm), which R's `openair::importImperial()` converts on read. Aeolus read the raw file and labelled it `ug/m3`, so NO2 and NOx were low by 1.91×, O3 by 2.00×, SO2 by 2.66× and CO by 1.16× — for every year of LAQN data, in every release since the RData feed was adopted (0.4.5). Marylebone Road's 2023 mean NO2 read 21.8 instead of 41.6. Aeolus now converts with Defra's published 20 °C factors (1.9125, 1.9957, 2.6609, 1.1642), which reproduce the AURN twin files exactly for ratified years; a live conformance test pins `LAQN` to `AURN` at Marylebone Road. `LAQN-ERG` was always correct. Particulates are unaffected. **Anything that stored LAQN gas values must re-download them.** A units audit of the other routes (stoichiometry test on every RData host and all six LMAM providers; Sonitus against EEA twins) found no other case.
- **EEA `ratification` labels were inverted** — the EIONET vocabulary is 1 = Verified, 2 = Preliminary verified, 3 = Not verified; Aeolus mapped 1 → `Provisional` and 2/3 → `Verified`. Under 0.4 (which queried only the up-to-date feed, all code 2) every EEA row was therefore labelled `Verified`; those rows are now `Provisional`, and code-1 rows from the verified archive are `Ratified`. Anything filtering EEA data on `ratification == "Verified"` was selecting exactly the unverified rows.
...
- **`find_sites()` listed long-closed AURN-family sites, many times each.** The openair metadata has one row per site-parameter, with `end_date` for series that have ended; Aeolus listed every row, so the nearest AURN site to central London was one closed in 1978, repeated once per pollutant. Sites whose every series has ended are now dropped (as openair's `importMeta(all = FALSE)` does) and metadata has one row per site, with `start_date` the earliest series and `end_date` `ongoing` while any series runs; the per-parameter `ratified_to` column is no longer in the site list (each row's `qa_code` carries that information). `find_sites(..., include_closed=True)` / `networks.get_metadata(..., include_closed=True)` keep them.
...
- **EEA downloads for earlier years came back empty.** Aeolus asked the EEA download API only for dataset 1, believing it to be the verified archive; it is the *up-to-date* feed (recent months). Every download now queries the verified archive and the up-to-date feed (and Airbase for windows reaching 2012 or earlier), serves each site-pollutant-day from the highest-priority dataset that holds it, and records which in `backend` (`EEA_E1A`, `EEA_E2A`, `EEA_AIRBASE`). Clocks are handled per dataset and country: Ireland's verified archive is on plain UTC, unlike the EEA's stated UTC+1, and Italy's up-to-date feed is on local time. EEA remains `experimental`: those clocks are verified for a handful of countries only.
...
- **OpenAQ downloads failed on a fresh install** — OpenAQ SDK 1.0.0 final moved `OpenAQError` from `openaq.shared.exceptions` to `openaq.core.exceptions`. Aeolus requires `openaq>=1.0.0rc2` but imported the release-candidate path, so any environment resolving the current SDK raised `ModuleNotFoundError` inside `download("OPENAQ", ...)`. Both locations are now supported. (Found by the new CI job; verified live against SDK 1.1.0.)
- **The cache could keep serving values this release corrects.** Cache entries now live under a versioned directory (`~/.cache/aeolus/v4/`), so Parquet files written by earlier versions — LAQN in ppb, inverted EEA labels, sentinel rows — are never served again. `cache_info()["legacy_files"]` counts them and `clear_cache()` removes them. Also: a date range that ended *after* it was fetched is refreshed like a rolling window rather than kept forever; a rolling window's TTL scales with its length (`last="2h"` refreshes every 12 minutes, not hourly); entries are written atomically, and an unreadable entry is a miss rather than a crash.
- **`summarise()` over-reported data capture for UK networks.** Sources now drop missing values, so a frame spans only first-to-last *valid* reading — an analyser that died in week one read as 100 % capture. `download()` records the requested range on the frame (`df.attrs["aeolus_requested_range"]`) and `summarise()` measures capture against it.
- **`aqi_summary()` coverage** inferred the data's cadence separately in each period, so a day with two readings twelve hours apart read as complete 12-hourly data. Cadence is now inferred once per site and pollutant.
- **AirQo's API token could appear in warnings.** AirQo authenticates in the query string and `requests` puts the URL in its error messages; metadata and grid failures now report the error type and HTTP status only. AirQo site paging advances by the number of sites returned (not requested) and warns if the listing comes back short; existing `latitude`/`longitude` are no longer overwritten with NaN.
...
- **Real QA codes.** `qa_code` is now the upstream's own token for the AURN family (`verified`/`unverified` for AURN, `Ratified`/`Provisional` for SAQN, WAQN, NI and AQE — derived per site and pollutant from the openair metadata's `ratified_to` date), for the SOS near-real-time sources (always the unratified token), EEA (the EIONET `Verification` code, `"1"`/`"2"`/`"3"`), PurpleAir (the channel/confidence label), Breathe London (`RatificationStatus`, null where absent) and AirNow (`Provisional`). LAQN, LMAM, Sensor.Community, Sonitus, OpenAQ and AirQo remain `unknown`: those feeds publish no per-row flag (AirQo's may; it is uncharacterised until the key is renewed). `Supplied` is in the UK vocabularies but is never emitted yet — the metadata carries no signal for it.
- **The `ratification` mirror is now derived from the QA columns, and changes for wired networks:** AURN family `None` → `Ratified`/`Provisional`; EEA (inverted in 0.4) `Verified` → `Provisional` for codes 2/3 and `Provisional` → `Ratified` for code 1; PurpleAir `Single Channel (A/B)` → `Validated` and `Channel Disagreement`/`Sensor Saturation`/`Invalid` → `Invalid`; Breathe London `Indicative`/`P` → `Unvalidated`/`Provisional`. The verbatim token is always in `qa_code`.
- A wired adapter returns `types.ADAPTER_DATA_COLUMNS_QA` (the old eight plus `qa_code`). Each AURN-family download now also fetches that network's metadata once per process for the `ratified_to` join.
...
- The cache moves to `~/.cache/aeolus/v4/`; nothing written by an earlier version is served.
...
- **Source status.** `aeolus.get_source_info()` now reports `status` (`"stable"` or `"experimental"`) and a `status_note`. An experimental source raises one `AeolusExperimentalWarning` per process on first use. **EEA is marked experimental**: its timestamp clocks (UTC+1, with per-country exceptions) are verified for a handful of countries only.
- NOx (`NOX`, `NOXasNO2`) now has a molecular weight (as NO2, by convention), so NOx in ppb can be converted by `aq_stats()` and friends.
